...
def fetch_and_process_one(exchange, usdt_rates):
    ...
    # send message with ohlc prices to Core app

    #publish_message_to_queue(message=json.dumps(tickers.symbols_info), topic_arn=AWS_SNS_TOPIC_ARN)
    #publish_message_to_queue(message=json.dumps(tickers.symbols_info), topic_arn=AWS_SNS_TOPIC_ARN, subject="ohlc prices")
